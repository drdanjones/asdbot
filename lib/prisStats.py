...
def respond(command):
    k = handleResponse(command)
    if len(k) > 10:
        response = k
    else:
        # Copied from old acebot.py
        # slack_client.api_call("chat.postMessage", channel=channel, text="Total prisoners: " + str(k[0]+k[1]), as_user=True)
        # slack_client.api_call("chat.postMessage", channel=channel, text="Male prisoners: " + str(k[0]), as_user=True)
        # slack_client.api_call("chat.postMessage", channel=channel, text="Female prisoners: " + str(k[1]), as_user=True)

        response = []
        response.append("Total prisoners: " + str(k[0] + k[1]))
        response.append("Male prisoners: " + str(k[0]))
        response.append("Female prisoners: " + str(k[1]))

        response.append("Figures correct as of " + str(k[2]))
    if not k[3]:
...
prison_stats = botguts.Bot_Command('prison population', respond)
bot_commands.append(prison_stats)
